...
Pokedex::Pokedex(vector<vector<string>> _data, int _width, int _height){
    ...
    menuBox.setPosition(sf::Vector2f(0,height));
    display = false;
    page = 1;
...
void Pokedex::setDisplay(bool _display){
    display = _display;
}
void Pokedex::toggleDisplay(){
    display ^= true;
}
bool Pokedex::getDisplay(){
    return display;
}
int Pokedex::getPage(){
    return page;
